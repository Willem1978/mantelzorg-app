...
VALUES
  ...
  (gen_random_uuid()::text, 'HULP_MANTELZORGER', 'emotionele-steun', 'Praten, steun & lotgenoten', '💚', 'Praten, steun & lotgenoten', 3, true, now(), now()),
  (gen_random_uuid()::text, 'HULP_MANTELZORGER', 'leren-en-training', 'Leren & training', '🎓', 'Leren & training', 4, true, now(), now())
ON CONFLICT (type, slug) DO UPDATE SET
  naam = EXCLUDED.naam, icon = EXCLUDED.icon, hint = EXCLUDED.hint,
